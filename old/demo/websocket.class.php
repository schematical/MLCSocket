...
class WebSocket{
  ...
  function CheckFile(){
  	$strFileLoc = dirname(__FILE__) . '/x.txt';
  	$strTxt = file_get_contents($strFileLoc);
  	$this->say("Message:" . $strTxt);
  	if(strlen($strTxt) > 0){
  		$this->broadcast($strTxt);
  		file_put_contents($strFileLoc, '');
  	}
  }
  ...
  function broadcast($strText){
  	foreach($this->users as $intIndex => $objUser){
  		$this->send($objUser->socket,$strText);
  	}
  }
